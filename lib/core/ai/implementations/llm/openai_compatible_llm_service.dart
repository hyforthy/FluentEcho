...
class OpenAICompatibleLLMService implements ILLMService {
  ...
  Stream<String> optimizeTextStream({
    ...
    if (!isConfigured) {
      throw const MissingKeyException('LLM service is not configured');
    }

    final prompt = _buildOptimizePrompt(text, detectedLanguage);
    yield* _streamChat(prompt);
  }
  ...
  Future<String> translate({
    ...
    await for (final delta in _streamChat(prompt)) {
      buffer.write(delta);
    }
  ...
  Future<String> detectLanguage(String text) async {
    ...
    try {
      ...
      if (choices == null || choices.isEmpty) return 'zh';
      final message =
          (choices[0] as Map<String, dynamic>)['message'] as Map<String, dynamic>?;
      return (message?['content'] as String?)?.trim().toLowerCase() ?? 'zh';
  ...
  Future<List<String>> suggestTags({
    ...
    try {
      ...
      await for (final delta
          in _streamChat(prompt).timeout(const Duration(seconds: 10))) {
        buffer.write(delta);
      }
  ...
  Stream<String> _streamChat(String userPrompt) async* {
    try {
      ...
      if (e.type == DioExceptionType.connectionTimeout ||
        ...
        throw const TimeoutException('LLM request timed out');
      }
      throw NetworkException('LLM request failed: ${e.message}');
    }
  }
